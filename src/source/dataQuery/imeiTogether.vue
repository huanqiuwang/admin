				    ...
				    <div class="extra-table" :class="tableActiveClass">
						...
						<el-table 
				    		...
				    		:span-method="objectSpanMethod">
						   	...
						   	<el-table-column prop="target" :label="'采集时间\n(IMSI:'+tableAjax.imsi+')'" :formatter="hqDateFormat1"></el-table-column>
 	...
 	export default {
	    ...
	    methods: {
	    	pageInit: function(){
				...
				this.tableAjax.imsi = jsondata && jsondata.length > 0 ? (jsondata[0].IMSI || '--') : '--';
            ...
            showTable: async function(row, column, cell, event){
            	var _this = this,url,e;

                this.extraTableList = ["hidden","hidden","hidden","hidden"];

	   			if(column.label == "时间匹配度"){
	   				this.tableActiveClass = 'active';
	   				e = await loading(AJAX.post(_this.ajaxConf.url5+'/'+row.imsi+'/'+_this.tableAjax.interaval,{key: _this.searchKey}));
	    ...
	    watch: {
	    	...
	    	value: function(){
	    		this.tableAjax.type = this.value
	    	},
	    	unionKey: function(){
	    		this.pageInit();
	    	},
