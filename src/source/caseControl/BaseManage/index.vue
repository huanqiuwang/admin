<template>
	<div class="search-wrapper case-table">
		...
		<!-- 案件列表查询 -->
		<div class="case-query-list temprary-query-list">
			...
			<span class="btn query-btn" @click="queryInit">查询</span>
			<span class="btn reset-btn" @click="reset">重置</span>
		</div>

		<!-- 批量操作 -->
		<div class="case-add-case">
			<span class="btn query-btn" @click="showAdd">添加案件</span>
		...
		<div class="case-total-info">
			...
			<span>共有<i>{{pageSplit.total}}</i>项布控，<label>{{runtime}}</label>运行中，<label>{{outtime}}</label>已停止</span>
		</div>
		...
		<div class="search-table">
			...
			<el-table 
				...
				<el-table-column prop="monitorvalue" label="布控字段"></el-table-column>
		  	...
		  	<el-pagination
	            ...
	            :current-page="pageSplit.currentPage"
	            :page-sizes="pageSplit.pageSizeList"
	            :page-size="pageSplit.pageSizeNow"
	            layout="total, sizes, prev, pager, next, jumper"
	            :total="pageSplit.total">
	        </el-pagination>
	        <div class="search-coperation bottom-coperation"></div>
        </div>
		...
		methods: {
			...
			dataInit: async function(page_now,page_size,refresh){
				...
				if(res.length == 0){
					...
					this.pageSplit.total = 0;
				}else{
                	if(!refresh){
                       	this.pageSplit.total = res.page.totalCount;
                    	this.pageSplit.pageSizeNow = this.searchCache.pageSize;
                    }
				}
			},
