<template>
  <ty-map :center="center">
    ...
    <ty-map-sider v-else slot="sider" v-model="keyword" @onSearch="onSearchPoint" @onClear="onSearchPoint">
      <sider-point v-for="point of filteredSiderPoints" :key="point.carplate" :car="em(point.carplate)" :point="point" :highlight="siderHighlightId === point.carplate" @onClick="onClickSiderPoint(point)" @onClickDetail="onClickSiderDetail(point)" />
    </ty-map-sider>
  </ty-map>
</template>
  ...
  export default {
    ...
    computed: {
      ...
      siderHighlightPoint () {
        ...
        return this.filteredSiderPoints.find(({ carplate }) => carplate === this.siderHighlightId)
      },
      unionKey: function(){
        return this.pkey + this.cars;
      }
    },
    watch: {
      unionKey (nv, ov) {
      ...
      filteredKeyword (nv, ov) {
        let keyword = nv.trim()
        if (keyword !== ov.trim()) {
          let filteredSiderPoints = keyword ? this.siderPoints.filter(({ carplate }) => carplate.indexOf(keyword) !== -1) : this.siderPoints
          if (this.siderHighlightId !== undefined) {
            let index = filteredSiderPoints.findIndex(({ carplate }) => carplate === this.siderHighlightId)
            if (index === -1) {
    ...
    methods: {
      ...
      async loadData () {
        if (this.pkey && this.pkey.trim() && this.cars && this.cars.trim()) {
          let { result: siderPoints } = await AJAX.post('search/map/data/carplate/location/list', {
            mapKey: this.pkey,
            list: this.cars
          })
      ...
      async getTargetDetailPoints () {
        ...
        if (!points) {
          ...
          let { result } = await AJAX.post(`search/map/data/carplate/location/info/list`, { mapKey: this.pkey, carplate: this.siderHighlightId }, { cancelToken: this.cancel.token })
      ...
      onClickSiderPoint ({ carplate, list }) {
        if (this.siderHighlightId !== carplate) {
          this.beforeSiderHighlightIdChange()
          this.siderHighlightId = carplate
          this.points = list
