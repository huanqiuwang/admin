    ...
    methods: {
      ...
      async loadData () {
        if (this.pkey && this.pkey.trim()) {
          let { MAC, addressNumber, collectNumber, areaNumber, result: points } = await AJAX.post('search/map/data/mac/location/list', {
            mapKey: this.pkey
          })
          this.info = { data: MAC, addressNumber, collectNumber, areaNumber }
          this.points = points
      ...
      async onClickDetail (point) {
        ...
        if (!this.detailMode || this.detailPoint.deviceId !== point.deviceId) {
          ...
          let { result = [] } = await AJAX.post('search/map/data/mac/location/info/list', { mapKey: this.pkey, deviceId: point.deviceId }, { cancelToken: this.cancel.token })
