<template>
  <ty-map :center="center">
    ...
    <ty-map-sider slot="sider" :loading="loading" v-model="keyword" @onSearch="onSearch" @onClear="onSearch">
      <div class="sider-header" slot="footer-header">IMEI：{{ imei }}</div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async loadData () {
        if (this.pkey && this.pkey.trim() && this.imeis && this.imeis.trim()) {
          this.loading = true
          try {
            let { data: imei, result: points } = await AJAX.post('search/map/data/imei', {
              mapKey: this.pkey,
              list: this.imeis
            })
