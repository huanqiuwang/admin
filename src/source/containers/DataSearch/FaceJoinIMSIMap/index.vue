<template>
  <ty-map :center="center">
    ...
    <ty-map-sider v-else slot="sider" v-model="keyword" @onSearch="onSearchPoint" @onClear="onSearchPoint">
      <sider-point v-for="point of filteredSiderPoints" :key="point.DATA" :imsi="em(point.DATA)" :point="point" :highlight="siderHighlightId === point.DATA" @onClick="onClickSiderPoint(point)" @onClickDetail="onClickSiderDetail(point)" />
    </ty-map-sider>
  </ty-map>
</template>
    ...
    methods: {
      ...
      async loadData () {
        if (this.pkey && this.pkey.trim() && this.imsis && this.imsis.trim()) {
          let { result: siderPoints } = await AJAX.post('contact/map/list/imsi', {
            mapKey: this.pkey,
            list: this.imsis
          })
      ...
      async getTargetDetailPoints () {
        ...
        if (!points) {
          ...
          let { result } = await AJAX.post(`contact/map/info/imsi`, { mapKey: this.pkey, data: this.siderHighlightId }, { cancelToken: this.cancel.token })
