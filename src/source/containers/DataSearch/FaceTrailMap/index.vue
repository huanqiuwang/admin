<template>
  <ty-map :center="center">
    ...
    <ty-map-sider slot="sider" :loading="loading" v-model="keyword" @onSearch="onSearch" @onClear="onSearch">
      <sider-point v-for="({ id, time, address, score, face }, index) of filteredPoints" :key="id" :index="index + 1" :time="em(time)" :address="em(address)" :score="score" :face="face" :highlight="highlight === index" @onClick="onClickPoint(index)" />
    </ty-map-sider>
    <trail-tip class="tip" />
  </ty-map>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      async loadData () {
        this.loading = true
        try {
          let { result: points } = await AJAX.post('face/getTimeFaceMap', {
            mapKey: this.pkey,
            list: this.faces
          })
          points = (points || []).map(({ captureId, lon, lat, address, time, score, captureImgFullUrl }) => ({ id: captureId, lng: Number(lon), lat: Number(lat), address, time: format(Number(time), 'YYYY-MM-DD HH:mm:ss'), score: percentage(score), face: captureImgFullUrl }))
          this.points = points
