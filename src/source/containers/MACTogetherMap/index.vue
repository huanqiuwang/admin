<template>
  <ty-map :center="center">
    ...
    <ty-map-sider v-else slot="sider" v-model="keyword" @onSearch="onSearchPoint" @onClear="onSearchPoint">
      <sider-point v-for="point of filteredSiderPoints" :key="point.mac" :mac="em(point.mac)" :point="point" :highlight="siderHighlightId === point.mac" @onClick="onClickSiderPoint(point)" @onClickDetail="onClickSiderDetail(point)" />
    </ty-map-sider>
  </ty-map>
</template>
  ...
  export default {
    ...
    computed: {
      ...
      siderHighlightPoint () {
        ...
        return this.filteredSiderPoints.find(({ mac }) => mac === this.siderHighlightId)
      },
      unionKey: function(){
    ...
    watch: {
      ...
      filteredKeyword (nv, ov) {
        let keyword = nv.trim()
        if (keyword !== ov.trim()) {
          let filteredSiderPoints = keyword ? this.siderPoints.filter(({ mac }) => mac.indexOf(keyword) !== -1) : this.siderPoints
          if (this.siderHighlightId !== undefined) {
            let index = filteredSiderPoints.findIndex(({ mac }) => mac === this.siderHighlightId)
            if (index === -1) {
    ...
    methods: {
      ...
      async loadData () {
        if (this.pkey && this.pkey.trim() && this.macs && this.macs.trim()) {
          let { result: siderPoints } = await AJAX.post('mac/together/map', {
            key: this.pkey,
            macs: this.macs
          })
      ...
      async getTargetDetailPoints () {
        ...
        if (!points) {
          ...
          let { result } = await AJAX.post(`mac/together/info`, { rids: this.siderHighlightPoint.rids }, { cancelToken: this.cancel.token })
      ...
      onClickSiderPoint ({ mac, deviceList }) {
        if (this.siderHighlightId !== mac) {
          this.beforeSiderHighlightIdChange()
          this.siderHighlightId = mac
