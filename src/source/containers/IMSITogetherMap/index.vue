<template>
  <ty-map :center="center">
    ...
    <ty-map-sider v-else slot="sider" v-model="keyword" @onSearch="onSearchPoint" @onClear="onSearchPoint">
      <sider-point v-for="point of filteredSiderPoints" :key="point.imsi" :imsi="em(point.imsi)" :point="point" :highlight="siderHighlightId === point.imsi" @onClick="onClickSiderPoint(point)" @onClickDetail="onClickSiderDetail(point)" />
    </ty-map-sider>
  </ty-map>
</template>
  ...
  export default {
    ...
    computed: {
      ...
      siderHighlightPoint () {
        ...
        return this.filteredSiderPoints.find(({ imsi }) => imsi === this.siderHighlightId)
      },
      unionKey: function(){
        return this.pkey + this.imsis;
      }
    },
    watch: {
      unionKey (nv, ov) {
      ...
      filteredKeyword (nv, ov) {
        let keyword = nv.trim()
        if (keyword !== ov.trim()) {
          let filteredSiderPoints = keyword ? this.siderPoints.filter(({ imsi }) => imsi.indexOf(keyword) !== -1) : this.siderPoints
          if (this.siderHighlightId !== undefined) {
            let index = filteredSiderPoints.findIndex(point => point.imsi === this.siderHighlightId)
            if (index === -1) {
    ...
    methods: {
      ...
      async loadData () {
        if (this.pkey && this.pkey.trim() && this.imsis && this.imsis.trim()) {
          let { result: siderPoints } = await AJAX.post('imsi/together/map', {
            key: this.pkey,
            imsis: this.imsis
          })
      ...
      em (imsi) {
        return this.filteredKeyword ? imsi.replace(this.regexp, `<em class="em">${this.filteredKeyword}</em>`) : imsi
      },
      ...
      async getTargetDetailPoints () {
        ...
        if (!points) {
          ...
          let { result } = await AJAX.post(`imsi/together/info`, { rids: this.siderHighlightPoint.rids }, { cancelToken: this.cancel.token })
          this.cancel = undefined
          points = result.map(({ address, capturetime, lon, lat }) => ({ address, time: format(capturetime * 1000, 'YYYY-MM-DD HH:mm:ss'), lng: Number.parseFloat(lon), lat: Number.parseFloat(lat) }))
      ...
      onClickSiderPoint ({ imsi, deviceList }) {
        if (this.siderHighlightId !== imsi) {
          this.beforeSiderHighlightIdChange()
          this.siderHighlightId = imsi
          this.points = deviceList
          this.filteredPoints = deviceList
          if (deviceList && deviceList.length) {
            let { lon: lng, lat } = deviceList[0]
            this.center = { lng, lat }
          }
        }
      },
