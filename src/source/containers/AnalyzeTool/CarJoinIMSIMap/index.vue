<template>
  <ty-map :center="center">
    ...
    <ty-map-sider v-else slot="sider" v-model="keyword" @onSearch="onSearchPoint" @onClear="onSearchPoint">
      <sider-point v-for="point of filteredSiderPoints" :key="point.target" :imsi="em(point.target)" :point="point" :highlight="siderHighlightId === point.target" @onClick="onClickSiderPoint(point)" @onClickDetail="onClickSiderDetail(point)" />
    </ty-map-sider>
  </ty-map>
</template>
    ...
    methods: {
      ...
      async loadData () {
        if (this.pkey && this.pkey.trim() && this.imsis && this.imsis.trim()) {
          let { result: siderPoints, sessionKey } = await AJAX.post('relate/carplate-imsi-map', {
      ...
      async getTargetDetailPoints () {
        ...
        if (!points) {
          ...
          let { result, sessionKey } = await AJAX.post(`relate/carplate-imsi-map-detail`, {
            sessionKey: this.pkey,
            imsi: this.siderHighlightId,
            carplate: this.car,
            list: this.areas,
            time: this.time
          }, {
