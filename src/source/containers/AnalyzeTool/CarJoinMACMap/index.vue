<template>
  <ty-map :center="center">
    ...
    <ty-map-sider v-else slot="sider" v-model="keyword" @onSearch="onSearchPoint" @onClear="onSearchPoint">
      <sider-point v-for="point of filteredSiderPoints" :key="point.target" :mac="em(point.target)" :point="point" :highlight="siderHighlightId === point.target" @onClick="onClickSiderPoint(point)" @onClickDetail="onClickSiderDetail(point)" />
    </ty-map-sider>
  </ty-map>
</template>
    ...
    methods: {
      ...
      async loadData () {
        if (this.pkey && this.pkey.trim() && this.macs && this.macs.trim()) {
          let { result: siderPoints, sessionKey } = await AJAX.post('relate/carplate-mac-map', {
      ...
      async getTargetDetailPoints () {
        ...
        if (!points) {
          ...
          let { result, sessionKey } = await AJAX.post(`relate/carplate-mac-map-detail`, {
            sessionKey: this.pkey,
            mac: this.siderHighlightId,
            carplate: this.car,
            list: this.areas,
            time: this.time
          }, {
