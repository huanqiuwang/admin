<template>
	<div class="ty-content tenant-content">
		<div class="ty-query-list">
			<span class="btn active" @click="showAdd">添加租户</span>
		</div>
		<div class="ty-total-info">
			<label class="el-icon-info"></label>
			<span>共用 <label>{{ total || 0 }}</label> 名用户,其中启用状态<label>{{ }}</label>名,禁用状态<label>{{ }}</label>名</span>
		</div>
		<div class="ty-table">
			<el-table
			    :data="tableData"
			    ...
			    <el-table-column prop="time" label="组织名称"></el-table-column>
			    <el-table-column prop="time" label="身份证号码"></el-table-column>
			    <el-table-column prop="time" label="手机号码"></el-table-column>
			    <el-table-column prop="time" label="邮箱"></el-table-column>
			    <el-table-column prop="time" label="警号"></el-table-column>
			    <el-table-column prop="remark" label="备注"></el-table-column>
			    <el-table-column label="操作" width="150">
			    	<template slot-scope="scope">
			    		<el-button size="mini" @click="edit(scope.$index, scope.row)">编辑</el-button>
						<el-button size="mini" @click="passReset(scope.$index, scope.row)">密码重置</el-button>
			    	</template>
			    </el-table-column>
		  	</el-table>
		</div>
